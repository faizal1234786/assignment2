...
def findErrorClass( X, k ):
	...
	model = pickle.load(open("C:\\Users\\swift 5 1\\Downloads\\model.npz", 'rb'))
	...
	b = np.insert(b, 32, 0,axis=1)
	b = np.insert(b, 35, 0,axis=1)
	b = np.insert(b, 37, 0,axis=1)
	yPred = np.zeros((n, k))
	#print("y",yPred.shape)
	for i in range(n):
		yPred[i,:] = np.argsort(b[i,:])[-precsion_value_k:]
		yPred[i,:]= yPred[i,:][::-1]
	#	precision_array.append(y_pred)
	print(yPred.shape)
	return yPred
